$(document).ready(function() {
    ...
    $('body').on('hidden.bs.modal', '.proposal-convert-modal', function(event) {
        var proposal_pipeline_modal = $('.proposal-pipeline-modal:visible').length;
        // Fix for scroll
        proposal_pipeline_modal && $('body').addClass('modal-open');
    });
    ...
    setup: function(editor) {
        editor.addButton('save_button', {
            ...
            onclick: function() {
                ...
                $.post(admin_url + 'proposals/save_proposal_data', data).success(function(response) {
